...
define ks = Character('김승현')
define d = Character('댕댕이')
define l = Character('이민아')
# 여기에서부터 게임이 시작합니다.
label start:
    "서기 2186년 "
    scene bg lab1
...
label sangmul chater1:
    ...
    g "제가 알고있는 사람이 얼마나 많은데..."
    sm "최근 일주일이내에 만난 여성. 그중한명이라는데..."
    g "엄마?... 엄마는 아닐테고, 그럼 그 편의점에 있었던 그 여잔가."
    sm "어! 그래 그여자를 찾아가."
    g "아니 거의 일주일이 지났는데 어떯게 찾... 아 메일주소있다"
    sm "그 여자를 찾아서 나에게 알려주게."
    g "한번 해보겠습니다"
    g "'평범한 나에게 이런 일이 생길줄이야...'"
    jump sangmul ending1
    return

label sangmul ending1
    g "좋아 그여자랑 연락됐고, 이제 뭐하지?"
    g "그냥 자야지"
    "2시간후"
    "띠리리리ㅣㅣ리"
    g "오 생물학 교수님. 일은 잘 해결되었나요?"
    sm "당연하지. 그녀를 죽였고, 이제 지구는 무사하다."
    g "?"
    g "어째서 그런짓을?"
    sm "지구를 지킬려면 어쩔수 없었다. 그럼"
    "뚝"
    g "난 뭔짓을... 한거지."
    return
